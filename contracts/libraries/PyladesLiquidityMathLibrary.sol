...
library PyladesLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = PyladesLibrary.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, 'PyladesArbitrageLibrary: ZERO_PAIR_RESERVES');
        ...
        if (aToB) {
            uint256 amountOut = PyladesLibrary.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut = PyladesLibrary.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = PyladesLibrary.getReserves(factory, tokenA, tokenB);
        IPyladesPair pair = IPyladesPair(PyladesLibrary.pairFor(factory, tokenA, tokenB));
        bool feeOn = IPyladesFactory(factory).feeTo() != address(0);
